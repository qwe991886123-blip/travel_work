...
const schema = z.object({
  ...
  map_url: z.string().refine(v => !v || v.startsWith('http'), {
...
export default function AddSpotModal({ onClose }: AddSpotModalProps) {
  ...
  return (
    ...
    >
      <div className="flex max-h-[90vh] w-full max-w-xl flex-col overflow-hidden rounded-t-[32px] bg-white shadow-2xl sm:rounded-[32px]">
        {/* Header */}
        <div className="flex flex-shrink-0 items-start justify-between gap-4 px-8 pb-4 pt-8">
          <div>
            <h2 id="add-spot-title" className="text-2xl font-bold text-stone-900">
              Add New Spot
            </h2>
            <p className="mt-1 text-sm text-stone-500">Quickly save a place you want to visit.</p>
          </div>
          <button
            onClick={onClose}
            ...
            <div className="space-y-5">
              ...
              <div>
                ...
                {coverImage ? (
                  ...
                  <div
                    ...
                    onDrop={onDrop}
                    className={cn(
                      'flex aspect-video cursor-pointer flex-col items-center justify-center gap-2 rounded-2xl border-2 border-dashed transition',
                      isDragging
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="例如：梵魚寺"
                  className={cn(
                    'w-full rounded-2xl border px-4 py-3 text-sm outline-none transition focus:ring-2 focus:ring-stone-400/10',
                    errors.title
                ...
                />
                {errors.title && (
                  <p className="mt-1 text-xs text-rose-500" role="alert">{errors.title.message}</p>
                )}
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label htmlFor="spot-region" className="mb-2 block text-sm font-medium text-stone-700">
                    Region
                  </label>
                  <select
                    id="spot-region"
                    {...register('region_id')}
                    className="w-full rounded-2xl border border-stone-200 bg-white px-4 py-3 text-sm outline-none transition focus:border-stone-900 focus:ring-2 focus:ring-stone-400/10"
                  >
              ...
              {/* Map URL */}
              <div>
                <label htmlFor="spot-map" className="mb-2 block text-sm font-medium text-stone-700">
                  Google Map URL
                </label>
                <div className="flex gap-2">
                  <input
                    ...
                    className={cn(
                      'flex-1 rounded-2xl border px-4 py-3 text-sm outline-none transition focus:ring-2 focus:ring-stone-400/10',
                      errors.map_url
                        ? 'border-rose-300 focus:border-rose-400'
                        : 'border-stone-200 focus:border-stone-900'
                    )}
                ...
                {errors.map_url && (
                  <p className="mt-1 text-xs text-rose-500" role="alert">{errors.map_url.message}</p>
                )}
              </div>

              {/* Address */}
              <div>
                <label htmlFor="spot-address" className="mb-2 block text-sm font-medium text-stone-700">
                  Address
                </label>
                <input
                  id="spot-address"
                  {...register('address')}
                  placeholder="例如：부산 금정구 범어사로 250"
              ...
              {/* Description */}
              <div>
                <label htmlFor="spot-desc" className="mb-2 block text-sm font-medium text-stone-700">
                  Description
                </label>
                <textarea
                  id="spot-desc"
              ...
              {/* Notes */}
              <div>
                <label htmlFor="spot-notes" className="mb-2 block text-sm font-medium text-stone-700">
                  Notes
                </label>
                <textarea
                  id="spot-notes"
